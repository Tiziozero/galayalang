...
static inline int lexer_add_token(Lexer* l, Token t) {
    if (l->tokens_count >= l->max_tokens) {
        l->max_tokens *= 2;
        // info("increasing token space to %d...", l->max_tokens);
    ...
    // info("Token Count: %d. Max capacity: %d.",
...
// returns 1 on true
static inline int is_keyword(Span n1, Span* kws, int kwlen) {
...
static inline void handle_new_line(Lexer* lexer, char* buf, int* i,
int size, int* line, int* col, char* last_line) {
char* cur = &buf[*i];
if (c == '\n' || (buf[*i] == '\r' && buf[(*i)+1] == '\n')) {
(*column) = 1;
(*line)++;
...
static inline Lexer* lexer(char* buf, int size) {
    dbg("lexing file...");
    ...
    while (i < size) {
        ...
        if (c == '\n' || (buf[i] == '\r' && buf[i+1] == '\n')) {
            ...
            err( "Unknown kakapoopoo: c (ascii %u) in %d %d",  c, line, start_col);
            free(l);
            l = 0;
            return NULL;
        }
...
static inline int lexer_free(Lexer* lexer) {
    ...
    free(lexer->code);
    lexer->code = 0;
    for (int i = 0; i < lexer->lines_count; i++) {
