...
static inline Arena arena_new(int page_size, int item_size) {
    Arena a;
    ...
    a.offset = 0;
    dbg("New arena of page_size: %d (%d * %d)",
         a.page_size, page_size, item_size);
    return a;
}
...
static inline void* arena_alloc(Arena* a, int size) {
    ...
    if (a->offset + size > a->page_size) {
        ...
        if (a->current_page >= a->pages_count) {
            ...
            err("allocations/allocated %d/%d.", a->allocations, a->total_allocated);
            assert(0);
        }
    }
...
static inline void* _arena_add(Arena* a, int size, void* data) {
    if (size > a->page_size) {
        err("size too large");
        assert(0);
        return NULL;
