...
int symbols(Parser* p, SymbolTable* st, Node* n) {
    ...
    switch (n->kind) {
        case NodeVarDec:
            {
                ...
                if (!n->var_dec.type) {
                    ...
                    if (!symbols(p, st, n->var_dec.type)) {
                        panic("Failed to resolve vardec type data.");
                        return 0;
                    }
                    t = n->var_dec.type->type;
                    if (!t) {
                ...
                dbg("REsolved vardec type %zu", n->var_dec.value);
        ...
        case NodeArg:
            {
                ...
                if (!symbols(p, st, n->arg.type)) {
                    panic("failed to resolve arg type.");
                    return 0;
                }
                Type* t = n->arg.type->type;
                if (!t) {
                    panic("Failed to resolve arg type.");
                    errs++;
                    break;
                ...
                n->symbol = s;
                n->arg.ident->type = t;
                n->arg.ident->symbol = s; // set
                n->arg.ident->resolved = 1; // resolves
        ...
        case NodeStructDec:
            {
                ...
                Node* decs = n->struct_dec.field_decs; // NodeList
                                                       // add for using ptrs to struct in struct
                Type struct_type;
                ...
                Symbol* struct_t = complete_type(unfinished, t);
                unfinished->type = t;
                n->symbol = struct_t;
        ...
        case NodeTypeData:
            {
                Type* t = st_resolve_type(st, n->type_data);
                if (!t) {
        ...
        case NodeFieldDec:
            { 
                if (!symbols(p, st, n->field_dec.type)) {
                    panic("Faield to resolve type data.");
                    return 0;
                }
                if (n->field_dec.type->kind != NodeTypeData) {
                    panic("what");
                }
        ...
        case NodeFieldAccess:
            {
                if (!symbols(p, st, n->field_access.target)) {
